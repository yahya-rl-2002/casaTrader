...
interface OrganizedCompanyReportsProps {
...
export const OrganizedCompanyReports: React.FC<OrganizedCompanyReportsProps> = ({
  reports,
  companyName,
  companySymbol,
  onOpen,
  ...
  // Organiser aussi par type de rapport
  ...
  const getYearStats = (yearReports: Tables<'financial_reports'>[]) => {
    ...
    const resultsReports = yearReports.filter(r => r.report_type === 'resultats').length;
    const communiqueReports = yearReports.filter(r => r.report_type === 'communique').length;
    const otherReports = yearReports.filter(r => !['rapport-annuel', 'rapport-semestriel', 'rapport-trimestriel', 'resultats', 'communique'].includes(r.report_type || '')).length;
    
    return { annualReports, semestrialReports, quarterlyReports, resultsReports, communiqueReports, otherReports };
  };

  // Obtenir les initiales de l'entreprise
  const getCompanyInitials = () => {
    const parts = companyName.replace(/[^A-Za-zÀ-ÿ0-9 ]/g, "").trim().split(/\s+/);
        ...
        <div className="flex items-center gap-3">
          <Avatar className="h-12 w-12">
            <AvatarFallback className="bg-primary/10 text-primary font-semibold text-lg">
              {companySymbol.split(':')[1]?.substring(0, 2) || getCompanyInitials()}
            </AvatarFallback>
          </Avatar>
          <div>
              ...
              <div key={year} className="space-y-4">
                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {stats.resultsReports > 0 && (
                      <Badge variant="secondary" className="bg-purple-100 text-purple-800">
                        {stats.resultsReports} Résultat{stats.resultsReports > 1 ? 's' : ''}
                      </Badge>
                    )}
                    {stats.communiqueReports > 0 && (
                      <Badge variant="secondary" className="bg-gray-100 text-gray-800">
                        {stats.communiqueReports} Communiqué{stats.communiqueReports > 1 ? 's' : ''}
                      </Badge>
                    )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {yearReports.map((report) => {
                    const typeInfo = getReportTypeInfo(report.report_type || 'autre');
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="flex-1">
                                ...
                                {report.file_name && report.file_name !== report.title && (
                                  <p className="text-xs text-muted-foreground mt-1 line-clamp-1" title={report.file_name}>
                                    📄 {report.file_name}
                                  </p>
                                )}
